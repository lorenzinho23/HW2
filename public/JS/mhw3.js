...
function search(event) {
    ...
    if (!content) {     
        ...
        if (tipo === "dati") {
            fetch("mhw3Api/symbol/"+text).then(onResponse).then(onJSONDati);
        }
        
        if (tipo === "simbolo") {
            fetch("mhw3Api/name/"+text).then(onResponse).then(onJSONSimbolo); 
        }        
    }
}
...
function onJSONSimbolo(json) {
    ...
    for (let i=0; i<json.length; i++) {
        ...
        simbolo.textContent = "Simbolo:" + json[i].simbolo;
        const exchange = document.createElement("p");
        exchange.textContent = "Exchange:" + json[i].exchange;
...
function onJSONDati(json) {
    ...
    settore.textContent = "Settore:" + json.settore;
    const prezzo = document.createElement("p");
    prezzo.textContent = "Prezzo corrente:" + json.prezzo;
    const valuta = document.createElement("p");
    valuta.textContent = "Valuta:" + json.valuta;
    const high = document.createElement("p");
    high.textContent = "Massimo del giorno:" + json.high;
    const low = document.createElement("p");
    low.textContent = "Minimo del giorno:" + json.low;
    const open = document.createElement("p");
    open.textContent = "Apertura:" + json.open;
    const close = document.createElement("p");
    close.textContent = "Chiusura:" + json.close;
    const volume = document.createElement("p");
    volume.textContent = "Volume:" + json.volume;
...
function exchange(event) {
    ...
    if (!quantita) {     
        ...
        fetch("mhw3Api/da/" + da + "a/" + a + "quant/" + quant).then(onResponse).then(onJSONConv);
    }
}
...
function valori (event) {
    ...
    fetch("mhw3Api/da/" + by + "a/" + to).then(onResponse).then(onJSONMoneta); 
}
...
function onJSONConv(json) {
    ...
    quanto.textContent = "Quantità: " + valore + " " + da;
    const simbolo = document.createElement("p");
    ...
    risultato.textContent = "Valore: " + json.amount + " " + a;
