...
function aggiungiPreferiti(event) {
    ...
    fetch("mhw1Caricamento").then(onResponse).then(onJSONPreferiti);
    ...
    function onJSONPreferiti(json) {
        for (let i in json) {
            if (json[i].Codice == e.dataset.idPreferito) {
                ...
                croceP.src = "../public/foto/no.ico";
                fotoP.src = json[i].Logo;
                console.log(e.dataset.idPreferito);
                const id = e.dataset.idPreferito;
                fetch("mhw2Inserisci_preferiti/id/" + id);
...
function rimuoviPreferiti(event) {
    ...
    for (let i = 0; i<stella.length; i++) {
        if (stella[i].dataset.idPreferito === e.dataset.idCroce) {
            stella[i].src = "../public/foto/stellavuota.png";
            stella[i].addEventListener("click", aggiungiPreferiti);
            fetch("mhw2Rimuovi_preferiti/id/"+stella[i].dataset.idPreferito);
        }
    }    
...
fetch("mhw1Caricamento").then(onResponse).then(onJSONCaricamento);
...
function onJSONCaricamento(json) {
    ...
    for (let i=0;i<json.length;i++) {
        ...
        stella.src = "../public/foto/stellavuota.png";   
    ...
    if (id_session !== '') {
        fetch("mhw2Preferiti").then(onResponse).then(onJSONCaricaPreferiti);
    }
}
